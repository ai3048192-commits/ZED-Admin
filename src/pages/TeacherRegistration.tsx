...
const IMAGE_SLOTS: Record<ImageField, { folder: string; success: string; previewTitle: string }> = {
  ...
  id_front_url: {
    folder: "ids",
    success: "تم رفع وتحديث صورة وجه البطاقة في سجلات التحقق بنجاح!",
    previewTitle: "معاينة وجه البطاقة (التحقق)",
  },
  id_back_url: {
    folder: "ids",
    success: "تم رفع وتحديث صورة ظهر البطاقة في سجلات التحقق بنجاح!",
    previewTitle: "معاينة ظهر البطاقة (التحقق)",
  },
};
      ...
      >
        {src ? (
          ...
          <span className="flex flex-col items-center gap-2">
            ...
            <span className="text-xs text-slate-500">لم يتم رفع بطاقة التحقق بعد</span>
          </span>
        )}
        {uploading && (
...
export default function TeacherProfilePage({ userId: userIdProp }: { userId?: string }) {
  ...
  // حالة التحقق الخاصة بالمعلم (TeacherVerification Status)
  const isVerified = Boolean(idFrontImage && idBackImage);
  ...
  const handleSaveAll = async () => {
    ...
    try {
      ...
      setIsEditing(false);
      notify("success", "تم حفظ كل بيانات الصفحة وسجلات التحقق بنجاح في قاعدة البيانات!");
    } catch (err) {
  ...
  const handleDeleteRecord = async () => {
    ...
    const confirmed = window.confirm(
      "سيتم حذف ملفك الشخصي وبيانات التحقق بالكامل من قاعدة البيانات. لا يمكن التراجع عن هذه الخطوة. هل تريد المتابعة؟"
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setIsEditing(false);
      setRecordEditing(false);
      notify("success", "تم حذف السجل وبيانات التحقق نهائياً!");
    } catch (err) {
      notify("error", errorMessage(err));
    } finally {
      setRecordBusy(false);
    ...
    <div className={cx("min-h-screen space-y-6 text-slate-800", isEditing && "pb-28")} dir="rtl">
          ...
          <div className="flex flex-col items-center gap-6 text-center sm:flex-row sm:text-right">
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {/* حالة التحقق TeacherVerification Badge */}
                <span className={cx(
                  ...
                  <ShieldCheck size={13} />
                  {isVerified ? "حساب مُحقق (مكتمل البطاقة)" : "في انتظار استكمال التحقق من الهوية"}
                </span>
              </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3 lg:items-start">
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          {/* قسم التحقق من الهوية TeacherVerification */}
          <Panel id="TeacherVerification">
            <PanelHeader
              icon={<ShieldCheck size={22} className="text-indigo-600" />}
              title="التحقق من هوية المعلم (TeacherVerification)"
              subtitle="قم برفع صور البطاقة الشخصية (وجه وظهر) لاعتماد حسابك رسمياً."
            />
        ...
        >
          ...
          <form onSubmit={handleChangePassword} className="space-y-4">
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              >
                {changingPassword && <Loader2 size5={15} className="animate-spin" />}
                حفظ
              </button>
            </div>
          </form>
